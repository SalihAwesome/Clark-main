      ...
      <aside className="animate-slide-in relative flex h-full w-full max-w-2xl flex-col border-s border-line bg-surface/95 backdrop-blur-xl">
        <div className="flex items-center gap-3 border-b border-line px-5 py-4">
          {selected ? (
            ...
            <span className="flex items-center gap-2 text-sm font-bold uppercase tracking-[0.2em] text-accent"><Icon name="clock" size={16} /> History</span>
          )}
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {!selected ? (
            loading ? (
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Loading…</p>
            ) : list.length === 0 ? (
              <p className="text-xs uppercase tracking-wide text-muted-foreground">No conversations yet.</p>
            ) : (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex items-center gap-2 text-[10px] uppercase tracking-wide text-muted-foreground">
                        ...
                        <span className="flex items-center gap-1 rounded-full bg-line px-1.5 py-0.5 font-bold text-foreground"><Icon name={c.mode === "chat" ? "message" : "bot"} size={11} /> {c.mode === "chat" ? "Chat" : "Agent"}</span>
            ...
            <div>
              ...
              <div className="mb-4 flex items-center gap-2 text-[10px] uppercase tracking-wide text-muted-foreground">
                ...
                <span className="inline-flex items-center gap-1 rounded-full bg-line px-1.5 py-0.5 font-bold text-foreground"><Icon name={selected.mode === "chat" ? "message" : "bot"} size={11} /> {selected.mode === "chat" ? "Chat" : "Agent"}</span>
              ...
              {selected.mode === "chat" ? (
                  ...
                  {selected.transcript.map((e, i) =>
                    e.type === "prompt" ? (
                      <div key={i} className="flex justify-end">
                        <div className="max-w-[85%] rounded-2xl border border-accent/30 bg-accent/[0.08] px-3 py-2 text-sm text-foreground">
                          {String(e.content || "")}
                        </div>
                      </div>
                    ) : e.type === "answer" ? (
                ...
                <>
                  {(selected.prompts || []).map((p, i) => (
                    <div key={i} className="mb-3 rounded-2xl border border-accent/30 bg-accent/[0.08] px-3 py-2 text-sm text-foreground">
                      {p}
                    </div>
                  ))}
                  ...
                  {selected.final && (
                    <div className="mt-4 border-t border-line pt-3">
                      <div className="mb-1 text-[10px] font-bold uppercase tracking-[0.3em] text-accent">Final Answer</div>
